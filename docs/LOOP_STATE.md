...
| 5 | Governance core extraction: `governance/` package exposing critic + keyed chain + quarantine behind a documented, versioned interface (the GaaS seed; budget limiter stays app-coupled). | L | **done** (a0583e2 — facade, not physical move: components still depend on app internals, so relocation = churn without decoupling; standalone packaging gated on a real external consumer) |
...
| 6 | 2026-07-17 | #5 governance core | Done — app.governance facade (INTERFACE_VERSION 1.0.0): docstring = interface contract (fail-closed guarantees, semver rules, budget limiter excluded, honest portability caveats). Runner + main audit/quarantine endpoints import through it. Critic subagent caught a silent-green trap: retargeting runner's import would kill test_critic_policy's monkeypatch WITHOUT failing (real critic also reports v1) — fixed with sentinel policy_version=99 + retargeted patch; builder empirically confirmed the old target goes dead. 4 contract tests (identity, __all__, semver, tamper roundtrip). 254 passed / 2 skipped, verified independently. Commit a0583e2, pushed. |
